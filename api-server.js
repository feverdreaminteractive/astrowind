...
// Claude API endpoint that mirrors the Netlify function
app.post('/api/claude', async (req, res) => {
  try {
    console.log('API server received request:', req.body);

    // Parse request body
    const { message, browserData } = req.body;
    ...
    if (!apiKey) {
      ...
      return res.status(500).json({ error: 'AI service temporarily unavailable - no API key' });
    }

    // Create career-focused system prompt (same as Netlify function)
