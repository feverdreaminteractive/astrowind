...
export default async (req, context) => {
  ...
  try {
    ...
    const store = getStore('figma-cache');
    const cacheKey = `figma-${figmaFileId}`;

    // Check persistent cache unless force refresh
    if (!forceRefresh) {
      try {
        ...
        if (cached && cached.timestamp > Date.now() - CACHE_TTL) {
          ...
          return new Response(JSON.stringify(cached.data), {
            ...
            headers: {
              ...
              'X-Cache-TTL': Math.floor(CACHE_TTL / 1000)
            }
          });
        }
      } catch (e) {
    ...
    if (!figmaResponse.ok) {
      ...
      if (figmaResponse.status === 429) {
        // Check if we have any cached version (even expired)
        try {
          const staleCache = await store.get(cacheKey, { type: 'json' });
          if (staleCache) {
        ...
        return new Response(JSON.stringify({
          error: 'Figma API rate limit reached. Please try again later or use cached designs.',
          status: 429,
          suggestion: 'Try using a previously cached design or paste JSON directly.'
        }), {
          status: 429,
          headers: {
            ...CORS_HEADERS,
    ...
    try {
      ...
      console.log('Cached Figma data for:', figmaFileId);
    } catch (e) {
